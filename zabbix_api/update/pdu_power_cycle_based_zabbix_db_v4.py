...
def get_board_pdu_port(zapi, hostname):
    try:
        hosts = zapi.host.get({
            "output": ["hostid", "name"],
        ...
        if not type_full:
            print(f"No board PDU port data found for hostname: {hostname}")
            return None
        ...
        if match:
            ...
            print(f"No number found in board PDU port: {type_full}")
            return None
    except Exception as e:
        print(f"Error querying Zabbix for hostname {hostname}: {e}")
        return None
...
def create_parser():
    ...
    parser.add_argument("-hn", "--hostname", type=str, help="Hostname to check board PDU port (optional)", default=None)
...
async def main():
    try:
        ...
        board_port = None
        if args.hostname:
            board_port = get_board_pdu_port(zapi, args.hostname)
            print(f"Board port: {board_port}")

        if "lab" in args.ip or "ttm" in args.ip:
            ttm_name = check_ttm_device(args.ip)
            if ttm_name == 'ttm_gpio':
                if board_port is not None and args.action == "2":
                    print(f"Executing custom power cycle sequence for ttm_gpio: board_port {board_port}, outlet {args.outlet}")
                    run_on_ttm_gpio(args.ip, board_port, "0")  # Turn off board
                    run_on_ttm_gpio(args.ip, args.outlet, "0")  # Turn off host
                    time.sleep(POWER_CYCLE_DELAY)              # Wait
                    run_on_ttm_gpio(args.ip, board_port, "1")  # Turn on board
                    run_on_ttm_gpio(args.ip, args.outlet, "1") # Turn on host
                else:
                    run_on_ttm_gpio(args.ip, args.outlet, args.action)
            else:  # ttm_simple
                if board_port is not None and args.action == "2":
                    print(f"Executing custom power cycle sequence for ttm_simple: board_port {board_port}, outlet {args.outlet}")
                    run_on_ttm(args.ip, board_port, "0")       # Turn off board
                    run_on_ttm(args.ip, args.outlet, "0")      # Turn off host
                    time.sleep(POWER_CYCLE_DELAY)              # Wait
                    run_on_ttm(args.ip, board_port, "1")       # Turn on board
                    run_on_ttm(args.ip, args.outlet, "1")      # Turn on host
                else:
        ...
        if pdu_name == "Altuscn":
            if board_port is not None and args.action == "2":
                print(f"Executing custom power cycle sequence for Altuscn: board_port {board_port}, outlet {args.outlet}")
                run_on_altuscn_pdu(args.ip, board_port, "0")  # Turn off board
                run_on_altuscn_pdu(args.ip, args.outlet, "0") # Turn off host
                time.sleep(POWER_CYCLE_DELAY)                 # Wait
                run_on_altuscn_pdu(args.ip, board_port, "1")  # Turn on board
                run_on_altuscn_pdu(args.ip, args.outlet, "1") # Turn on host
            else:
                run_on_altuscn_pdu(args.ip, args.outlet, args.action)

        if pdu_name == "Aten":
            if board_port is not None and args.action == "2":
                print(f"Executing custom power cycle sequence for Aten: board_port {board_port}, outlet {args.outlet}")
                run_on_aten_pdu(args.ip, board_port, "0")     # Turn off board
                run_on_aten_pdu(args.ip, args.outlet, "0")    # Turn off host
                time.sleep(POWER_CYCLE_DELAY)                 # Wait
                run_on_aten_pdu(args.ip, board_port, "1")     # Turn on board
                run_on_aten_pdu(args.ip, args.outlet, "1")    # Turn on host
            else:
                run_on_aten_pdu(args.ip, args.outlet, args.action)

        if pdu_name == "Raritan":
            if board_port is not None and args.action == "2":
                print(f"Executing custom power cycle sequence for Raritan: board_port {board_port}, outlet {args.outlet}")
                run_on_raritan_pdu(args.ip, board_port, "0")  # Turn off board
                run_on_raritan_pdu(args.ip, args.outlet, "0") # Turn off host
                time.sleep(POWER_CYCLE_DELAY)                 # Wait
                run_on_raritan_pdu(args.ip, board_port, "1")  # Turn on board
                run_on_raritan_pdu(args.ip, args.outlet, "1") # Turn on host
            else:
                run_on_raritan_pdu(args.ip, args.outlet, args.action)

        if pdu_name == "Eaton":
            if board_port is not None and args.action == "2":
                print(f"Executing custom power cycle sequence for Eaton: board_port {board_port}, outlet {args.outlet}")
                run_on_eaton_pdu(args.ip, board_port, "0")    # Turn off board
                run_on_eaton_pdu(args.ip, args.outlet, "0")   # Turn off host
                time.sleep(POWER_CYCLE_DELAY)                 # Wait
                run_on_eaton_pdu(args.ip, board_port, "1")    # Turn on board
                run_on_eaton_pdu(args.ip, args.outlet, "1")   # Turn on host
            else:
