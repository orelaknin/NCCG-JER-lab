...
class ZabbixController:
    ...
    def __init__(self, zabbix_url: str = "http://ladjzabbixc.jer.intel.com/zabbix/",
                 username: str = "Del_Hosts", password: str = "$giga"):
    ...
    def power_control_command(self, hostname: str, action: str, component_type: str = "host") -> int:
        ...
        current_host = self._clean_hostname(self._current_hostname)
        target_host = self._clean_hostname(hostname)
        ...
        try:
            # Get host ID
            hostname_clean = self._clean_hostname(hostname)
    ...
    def get_item_lastvalue(self, hostname: str, item_name: str) -> Optional[str]:
        """
        Get the last value of a Zabbix item for a host

        Args:
            hostname: Hostname to get item value for
            item_name: Name or key of the Zabbix item to retrieve

        Returns:
            str: Item's last value if successful, None if failed
        """
        hostname_clean = self._clean_hostname(hostname)
        log(f"Getting item '{item_name}' last value for host: {hostname_clean}")
        ...
        try:
            ...
            # First try to search by item name
            ...
            # If not found by name, try searching by key
            if not items:
                items = self.zapi.item.get({
                    "output": ["itemid", "key_", "name", "lastvalue"],
                    "hostids": host_id,
            ...
            if not items:
                log(f"ERROR: Item '{item_name}' not found for host {hostname_clean}")
                return None

            # Get the last value from the item
            last_value = items[0].get('lastvalue', '').strip()
            item_key = items[0].get('key_', '')
            item_full_name = items[0].get('name', '')
            
            if last_value:
                log(f"Retrieved item '{item_full_name}' (key: {item_key}) value for {hostname_clean}: {last_value}")
                return last_value
            else:
                log(f"WARNING: Item '{item_full_name}' exists but has no value for {hostname_clean}")
                return None

        except Exception as e:
            log(f"ERROR: Failed to get item '{item_name}' for {hostname_clean}: {e}")
            return None

    def get_usb2lan_mac_address(self, hostname: str) -> Optional[str]:
        """
        Get the usb2lan MAC address for a host from Zabbix item

        Args:
            hostname: Hostname to get usb2lan MAC address for

        Returns:
            str: usb2lan MAC address if successful, None if failed
        """
        return self.get_item_lastvalue(hostname, "usb2lan mac")
    ...
    def _clean_hostname(self, hostname: str) -> str:
        """Remove domain suffixes from hostname (wrapper for utility function)"""
        return clean_hostname(hostname)

    def get_hostname_with_suffix(self, hostname: str) -> str:
        """
        Get hostname with appropriate domain suffix based on IP address

        Args:
            hostname: Hostname (with or without suffix)

        Returns:
            str: Hostname with correct domain suffix (.jer.intel.com for 10.12.x.x IPs, .iil.intel.com for others)
        """
        # First clean the hostname to remove any existing suffix
        clean_host = self._clean_hostname(hostname)
        
        # Get the IP address for this host using the Zabbix item
        ip_address = self.get_item_lastvalue(clean_host, "get_ip_address")
        
        if not ip_address:
            log(f"WARNING: Could not determine IP for {clean_host}, defaulting to .iil.intel.com")
            return f"{clean_host}.iil.intel.com"
        
        # Check if IP starts with 10.12
        if ip_address.startswith("10.12"):
            suffix = ".jer.intel.com"
            log(f"Host {clean_host} has IP {ip_address} (JER network)")
        else:
            suffix = ".iil.intel.com"
            log(f"Host {clean_host} has IP {ip_address} (IIL network)")
        
        return f"{clean_host}{suffix}"
