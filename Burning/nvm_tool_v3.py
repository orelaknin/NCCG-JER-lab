...
def install_kernel_from_tgz(tgz_path, auto_reboot=False):
    ...
    try:
        ...
        if install_success:
            ...
            # Copy the extracted folder to ~/kernel_extract_backup
            try:
                backup_dir = os.path.expanduser("~/kernel_extract_backup")
                os.makedirs(backup_dir, exist_ok=True)
                # Use target_kernel_version as folder name, sanitized if necessary
                if target_kernel_version:
                    # Replace invalid characters for directory names (e.g., '+', '.') with '_'
                    safe_kernel_version = target_kernel_version.replace('.', '_').replace('+', '_')
                    backup_path = os.path.join(backup_dir, safe_kernel_version)
                else:
                    # Fallback to temp directory basename if kernel version is not available
                    backup_path = os.path.join(backup_dir, os.path.basename(temp_extract_dir))
