...
def mount_remote_share(remote_path, mount_point, username=None, password=None):
    ...
    try:
        ...
        return True, f"Mounted remote share {remote_path} to {mount_point}"
    except subprocess.CalledProcessError as e:
        return False, f"Error mounting share: {e}"
...
class NVMToolGUI:
    ...
    def mount_share(self):
        self.update_text_area(f"Checking if {self.remote_path} is mounted at {self.mount_dir}...")
        # Check if mount point is already mounted
        mount_output = subprocess.run(["mount"], capture_output=True, text=True).stdout
        if self.mount_dir in mount_output:
            self.update_text_area(f"{self.mount_dir} is already mounted. Skipping remount.")
            self.is_mounted = True
            self.mount_status.config(text=f"Mounted: {self.remote_path}")
            self.select_file_button.config(state=tk.NORMAL)
            self.populate_file_list()
        else:
            self.update_text_area(f"Attempting to mount {self.remote_path} to {self.mount_dir}...")
    ...
    def populate_file_list(self):
        ...
        if not os.path.isdir(self.current_dir):
            self.update_text_area(f"Error: Directory '{self.current_dir}' does not exist.")
            return
        contents = os.listdir(self.current_dir)
        contents.sort()
        self.update_text_area(f"Contents of: {self.current_dir}")
    ...
    def burn_image(self):
        ...
        if not result:
            self.update_text_area("Burn operation cancelled.")
            return
        self.install_button.config(state=tk.DISABLED)
        self.burn_button.config(state=tk.DISABLED)
        self.select_file_button.config(state=tk.DISABLED)
