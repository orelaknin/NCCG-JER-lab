...
class NVMToolGUI:
    def __init__(self, root):
        ...
        self.selected_kernel_package = None
        ...
        # Buttons
        self.proceed_button = ttk.Button(self.kernel_frame, text="Proceed to Installation", command=self.install_kernel, state=tk.DISABLED)
        ...
        ttk.Label(self.extra2_frame, text="Extra 2 Functionality (To be implemented)").grid(row=0, column=0, padx=5, pady=5)
    ...
    def populate_kernel_list(self):
        ...
        for item in contents:
            ...
            file_type = "DIR" if os.path.isdir(full_path) else "KERNEL" if item.lower().endswith(('.tgz', '.tar.gz')) else "FILE"
    ...
    def on_kernel_option_select(self, event):
        ...
        if not selection:
            return
        selected_item = self.kernel_listbox.get(selection[0]).split(' - ')[0]
        self.selected_kernel_package = os.path.join(self.mount_dir, "Kernel", selected_item)
        self.update_text_area(f"Selected package: {selected_item}", self.kernel_text_area)
        self.proceed_button.config(state=tk.NORMAL if os.path.isfile(self.selected_kernel_package) and 
                                 self.selected_kernel_package.lower().endswith(('.tgz', '.tar.gz')) else tk.DISABLED)
    ...
    def install_kernel(self):
        if not self.selected_kernel_package:
            messagebox.showerror("Error", "No kernel package selected for installation.")
            return
    ...
    def run_install_kernel(self):
        install_kernel_from_tgz(self.selected_kernel_package, self.auto_reboot_var.get(), self.kernel_text_area)
        self.proceed_button.config(state=tk.DISABLED)
        self.burn_button.config(state=tk.DISABLED if not self.selected_file else tk.NORMAL)
        self.select_file_button.config(state=tk.NORMAL)
        self.show_kernel_frame()
